"""GR00T-N1.7 policy wrapper for Franka Panda in MechanicalSearchEnv.

Embodiment : OXE_DROID_RELATIVE_EEF_RELATIVE_JOINT
Video keys : exterior_image_1_left (overhead), wrist_image_left (wrist)
...
_VIDEO_BUF = 16      # delta_indices: [-15, 0] → 16-frame ring buffer
ACTION_HORIZON = 40
...
class FrankaGR00TPolicy:
    ...
    Call reset() at episode start, then get_action() every step.
    ...
    dict with keys (all np.float32):
        ...
        "gripper_position" : [ACTION_HORIZON, 1]  gripper target (absolute [0,1])
        "joint_position"   : [ACTION_HORIZON, 7]  joint position delta (relative, rad)
    """
    ...
    def get_action(
        ...
        # warm-start: pad with first frame until buffer is full
        ...
        # delta_indices [-15, 0] → frames[0] (oldest kept) and frames[-1] (current)
        ext_video = np.stack([frames[0], frames[-1]], axis=0)   # [2, H, W, 3]
        wri_video = np.stack([wrists[0], wrists[-1]], axis=0)   # [2, H, W, 3]

        # eef_9d: pos(3) + first two columns of rotation matrix(6) = 9
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    action = policy.get_action(rgb, eef_pos, eef_rotmat, gripper, joint_pos)
